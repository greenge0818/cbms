...
#主机列表
SUFFIX="pro"
DEFAULT_HOSTS="10.173.163.88,10.173.163.95"

USERNAME="root"
PASSWORD="Prcsteeljsb11"
#INSTANCE="tomcat"
TOMCAT_HOME="/opt/tomcat8-cbms"
BAK_DIR="/opt/backup/cbms/${SUFFIX}"
TARGET_HOME="/tmp/cbms/${SUFFIX}"

#SQL文件目录
SQL_FILE_DIR=${TARGET_HOME}/sql
SQL_BACK_DIR="/opt/cbms/db_update/${SUFFIX}"

#数据库配置信息
DB_SERVER="rdsz9a5lhjx4oik5n18z.mysql.rds.aliyuncs.com"
DB_SCHEMA="steel_cbms"
DB_USERNAME="steel_cbms"
DB_PASSWORD="prcsteelcbms"
DB_BACKUP_DIR="/opt/backup/database/${SUFFIX}"
##############################################################
...
#    SERVER=$1
    ...
    execute_log "mysqldump -h ${DB_SERVER} -u ${DB_USERNAME} -p${DB_PASSWORD} -R ${DB_SCHEMA} > ${DB_BACKUP_DIR}/aout_backup_${SUFFIX}_${DB_SCHEMA}_$(date "+%Y%m%d_%H%M%S").sql"
...
function db_upgrade() {
    ...
    if [ -z "${files}" ]; then
        ...
        do
            execute_log "mysql -h ${DB_SERVER} -u ${DB_USERNAME} -p${DB_PASSWORD} steel_cbms < ${file}"
            mysql -h ${DB_SERVER} -u ${DB_USERNAME} -p${DB_PASSWORD} steel_cbms < ${file}
        done
...
function shutdown_tomcat(){
    execute_log "ssh ${DESTINATION} ps -ef |grep ${TOMCAT_HOME}/ |grep -v 'grep'"
