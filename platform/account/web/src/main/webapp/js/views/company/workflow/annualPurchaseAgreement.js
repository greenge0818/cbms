...
$().ready(function () {
    ...
    $(document).on("click", "#btnPass", function () {
        cbms.confirm("确定通过审核吗？", null, function () {
            var data = {
                ...
                oldAnnualPurchaseAgreementStatus: $("#status").val(),
                newAnnualPurchaseAgreementStatus: getAuditPassNewStatusByOldStatus($("#status").val()),
        ...
        $(document).on("click", "#define", function () {
            ...
            if (declineReason != "") {
                var data = {
                    ...
                    auditType: $("#auditType").val(),
                    oldAnnualPurchaseAgreementStatus: $("#status").val(),
                    newAnnualPurchaseAgreementStatus: getAuditRefuseNewStatusByOldStatus($("#status").val()),
                    annualPurchaseAgreementDeclineReason: declineReason,
    ...
    $(document).on("click", "#btnBack", function () {
        window.location.href = Context.PATH + "/flow/annual/" + $("#auditType").val() + "/list.html";
    });
...
function audit(data) {
    $.ajax({
        type: "POST",
        url: Context.PATH + "/flow/annual/doaudit.html",
        data: data,
        dataType: "json",
