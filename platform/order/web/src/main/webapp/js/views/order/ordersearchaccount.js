...
$().ready(function () {
    ...
    $(document).on("click", "#searchAccountList li a", function () {
    	if ($("#buyer").attr("buyerType")=='clearAllowanceType') {
    		$("#buyer").val("");
        	$("#buyer").removeClass("temp-lin");
    	}
...
function onBlur(input){
	$(input).unbind("blur").blur(function(){
		...
		if(inputVal!==""){
		   ...
		   var sellerClient=$(input).val();
        try{
	        newOrderHint(sellerClient);
        }catch(err){
...
function searchAccountByName(inputBox) {
    ...
    if (accountType == null || accountType == undefined || accountType == "")
        accountType = "";
    var cardInfoStatus = $(inputBox).attr("cardinfostatus");
    if (cardInfoStatus == null || cardInfoStatus == undefined || cardInfoStatus == "")
        cardInfoStatus = "";

    $.ajax({
        type: 'post',
        url: Context.PATH + "/account/searchaccount.html",
        data: {
            accountName: name,
            type: accountType,
            ...
            if (result) {
                ...
                if ($(result.data) != null && $(result.data).length > 0) {
                    ...
                    for (var i = 0; i < $(result.data).length; i++) {
                        ...
                        searchHtml += " supplierLabel='" + result.data[i].supplierLabel + "'";
                    ...
                    if (flagIndex > -1) {
                        ...
                        $("#searchAccountList a").click(function(){
                           var sellerClient= $(this).attr("accountname");
...
function newOrderHint(sellerClient){
    
	var estimate = [true, ""];
    $.ajax({
        type: 'post',
        url: Context.PATH + "/sys/findhint.html",
        async : false,
        data: {
        	userId :$("#user").attr("userid"),
            accountName: sellerClient,
            type: "new_order"
        },
        error: function (s) {
        }
        , success: function (result) {
               if(result && result.success){
                   $("#new_order_hint").removeClass("none");
                   $("#new_order_hint span").text(result.data.description)
                  if(result.data.ext1=="0"){
                     // $("#submit").attr("disabled","disabled");
                      estimate[0]=false;
                      estimate[1]=result.data.description;
                  }
                  if(result.data.ext1=="1"){
                      $("#submit").attr("disabled",false);
                          estimate[0]=true;
                          estimate[1]=result.data.ext3;
                  }
               }else{
                   $("#new_order_hint").addClass("none");
                   $("#submit").attr("disabled",false);
               }
        }
