...
//买家审核通过
$("#audit-buyer-certi").click(function(){
    $.ajax({
        type: "POST",
        url: Context.PATH + '/order/certificate/submitCert.html',
        data: {
            status : 'APPROVED',
            certid : $("#buyercertid").val()
        },
        ...
        success:function(re){
            if (re) {
                if (re.success) {
                    cbms.alert(re.data);
                    $("#audit-buyer-certi").hide();
                    $("#noaudit-buyer-certi").hide();
                    $("#cancel-buyer-certi").show();
                }
                else {
                    cbms.alert(re.data);
                }
...
//买家审核不通过
$(document).on("click","#buyer_cert_pay_shut",function(){
	if (!setlistensSave("#cert_shut_editForm")) return;
	  $.ajax({
	        type: "POST",
	        url: Context.PATH + '/order/certificate/submitCert.html',
...
$("#noaudit-buyer-certi").click(function(){
	var html="<form id='cert_shut_editForm'><div class='dialog-m'><textarea name='cause' class='textarea' must='1' style='width:320px;height:100px;'placeholder='请填写关闭订单的理由！'></textarea><br>"+
    "<div class='btn-bar text-center'><button type='button'  class='btn btn-default btn-sm' id='window_close'>取消</button>&nbsp;"+
    "<button type='button'  class='btn btn-info btn-sm' id='buyer_cert_pay_shut'>确定</button></div> </div></form>";
	cbms.getDialog("不通过审核",html);
});
//家卖取消审核通过
...
//买家取消审核通过
$("#cancel-buyer-certi").click(function(){
    $.ajax({
        type: "POST",
        url: Context.PATH + '/order/certificate/submitCert.html',
        data: {
