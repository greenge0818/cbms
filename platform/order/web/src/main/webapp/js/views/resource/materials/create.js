/**
 * 材质管理页面新增 js
 * created by  peanut on 2016-1-13
 */

//绑定输入框focus事件
$(document).on("click", "input[search='nsort']", function () {
	var categoryMaterialId=$("#materialsForm").find('input[name="categoryMaterialId"]').val();
	if(!categoryMaterialId){
		 getNsort($(this));
	}
});

// 绑定选择品名单击事件
$(document).on("click", "#showLayer_nsort span a", function () {
    var showLayer = $("#showLayer_nsort");
    var input = "input[inputid='" + $(showLayer).attr("inputid") + "']";
    var nsortId = $(this).attr("nsortid");
    var nsortName = $(this).text();
    $(input).attr("nsortid", nsortId);
    $(input).val(nsortName);
    $(showLayer).remove();
});

//鼠标滑过大类，显示大类下的小类
var t;
$(document).on("mouseover", ".product-t-li",
    function () {
        var tab = this;
        t = setTimeout(function () {
            $(tab).parent().find('li').removeClass('hover');
            $(tab).addClass('hover');
            $('div[name="nsort"]').hide();
            $('div[name="nsort"]:eq(' + ($(tab).index()) + ')').show();
            return false;
        }, 200);
    }
);

$(document).on("mouseout", ".product-t-li",
    function () {
        clearTimeout(t);
    }
);

// 阻止事件继续冒泡
$(document).on("mousedown", "#showLayer_nsort", function (e) {
    var event = e || window.event;
    event.stopPropagation();
});

/**
 * 点击弹层以外的地方会使弹层消失
 */
$("body").click(function (event) {
    var t = event.target;
    if ($(t).closest('.form-item').length > 0) {
        return;
    }
    $(".show-layer").hide();
});

//保存
$("#saveBtn").on("click",formSubmit);

/**
* 关闭窗口
*/
$("#cancelBtn").on("click",function(){
	cbms.closeDialog();
});

// 获取品名数据
function getNsort(inputBox) {
    if (_materialMgtAttr.categoryData == null ) {
        $.ajax({
            type: 'post',
            url: Context.PATH + "/smartmatch/purchaseorder/GetSortAndNsort.html",
            data: {},
            dataType: "json",
            async : false,
            error: function (s) {
            }
            , success: function (result) {
                if (result.statusCode == 0) {
                	_materialMgtAttr.categoryData = result.data;
                    loadSortName(_materialMgtAttr.categoryData);
                }
                loadSortName(_materialMgtAttr.categoryData);
                showNsortLayer(inputBox);
            }
        });
    }
    else {
        loadSortName(_materialMgtAttr.categoryData);
        showNsortLayer(inputBox);
    }
}

//显示品名弹层在对应的输入框下
function showNsortLayer(inputBox) {
    // 通过唯一属性 inputId 来关联
    var showLayer = $("#showLayer_nsort" );
    var inputId = "nsort" + new Date().getTime();
    $(inputBox).attr("inputid", inputId);
    $(showLayer).attr("inputid", inputId);

    setLayerPosition($(inputBox), $(showLayer));
    controlLayerShow($(showLayer));
}

/**
 * 加载品名大类数据
 * @param data    品名数据
 */
function loadSortName(data) {
    $("#showLayer_nsort").remove();
    // <!--品名数据层  S-->
    var layerHtml = "<div class='show-layer product-bar none' id='showLayer_nsort'>";
    layerHtml += "<div class='product-t'>";
    layerHtml += "<ul class='product-t-ul'></ul>";
    layerHtml += "</div>";
    layerHtml += "</div>";
    $("body").append(layerHtml);
    // <!--品名数据层 E-->
    if (data != null) {
        var search_sorts = data;
        $(search_sorts).each(function (i, e) {
            //卷板，型材 横向列表
            var li = '<li class="product-t-li ' + (i === 0 ? 'hover' : '') + '"><a href="javascript:;" sortId="' + e.sortID + '">' + e.sortName + '</a>'
                + '<em class="icon down-redArr-icon"></em></li>';
            $("#showLayer_nsort .product-t .product-t-ul").append(li);//'<a href="javascript:void(0)" sortId="'+e.sortID+'"><font>'+e.sortName+'</font></a>');
            //以下：品名明细，包括 热轧+品名
            //最大的包裹div
            var classDiv = $('<div name="nsort"/>');
            $(e.classInfo).each(function (j, f) {
                var nsort = $('<div class="product-con f-clrfix bder-b-dashed"/>');
                //热轧，冷轧
                var className = $('<div class="product-con-sort f-fl"><span class="bold">' + (f.className === "" ? "" : f.className + ":") + '</span></div>');
                if (f.nsort.length > 0) {
                    //品名列表最外层div
                    var nsortDiv = $('<div class="product-con-bar f-fl"/>'),
                        rowSize = 6,
                        nsortPkg = "";
                    $(f.nsort).each(function (k, g) {
                        //每N行换行div
                        if (k % rowSize === 0) {
                            nsortPkg += '<div class="product-con-bar-list">';
                        }
                        nsortPkg += "<span><a nsortId='" + g.nsortID + "' href='javascript:;' spec='"+ JSON.stringify(g.specName)+"'>" + g.nsortName + '</a>'
                        +'</span>';
                        if ((k + 1) % rowSize === 0 || (k + 1) === f.nsort.length) {
                            nsortPkg += '</div>';
                        }
                    });
                    $(nsortDiv).append($(nsortPkg));
                    $(nsort).append(className);
                    $(nsort).append(nsortDiv);
                }
                $(classDiv).append(nsort);
            });
            $("#showLayer_nsort").append(classDiv);

        });

        $("div[name='nsort']:gt(0)").hide();
    } else {
        cbms.alert("加载品名大类发生错误:" + data.message);
    }
}
/**
 * 移除按钮disabled属性
 */
function removeBtnAttr(){
	$("#saveBtn").removeAttr("disabled");
}
/**
*材质添加提交表单
*/
function formSubmit(){
 	if (setlistensSave("#materialsForm")) {
 		//材质记录id
 		var materialId=$("#materialsForm").find('input[name="materialId"]').val();
 		//品名材质记录id
 		var categoryMaterialId=$("#materialsForm").find('input[name="categoryMaterialId"]').val();
 		//品名uuid
 		var categoryName=$("#category").val();
 		//品名名称
 		var categoryUuid=$("#category").attr("nsortid");
 		//材质名称
 		var materialName=$.trim($("#material").val());
 		//备注
 		var remark=$("#remark").val();
		
 		 $.ajax({
             type: "POST",
             url: Context.PATH + '/resource/materials/save.html',
             data: {
            	 materialId:materialId,
            	 categoryMaterialId:categoryMaterialId,
            	 categoryName: categoryName,
            	 categoryUuid: categoryUuid,
            	 materialName:materialName,
            	 remark:remark
             },
             dataType: "json",
             success: function (response, textStatus, xhr) {
                 if (response.success) {
                 	cbms.closeDialog();
                 	_materialMgtAttr.dt.fnDraw();
                 }
             	 cbms.alert(response.data,removeBtnAttr);
             }
         });
 		 return false;
	}
}