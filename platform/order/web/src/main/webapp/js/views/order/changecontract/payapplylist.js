    ...
    tradeTable = jQuery("#dynamic-table").DataTable({
        "ajax": {
            ...
            , data: function (d) {
                ...
                d.alterStatus = 'PENDING_APPLY,PAYED_DISAPPROVE';//待申请付款变更/付款申请审核不通过
...
function showOperationalByStatus(orderId){
    var relateURL = Context.PATH + "/order/changecontract/"+orderId+"/payapplydetail.html"; //申请付款订单
    var printURL =  Context.PATH + "/order/changecontract/print/contract.html?orderChangeId="+ orderId; //打印变更合同
    var s = "";
    if($("#permission_payapply").val() == 'true'){
