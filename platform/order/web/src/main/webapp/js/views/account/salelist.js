...
function initTable() {
    tradeTable = jQuery("#dynamic-table").DataTable({
        "ajax": {
            "url": Context.PATH + "/account/loadSaleData.html"
            , "type": "POST"
            , data: function (d) {
        ...
        , columnDefs: [
            {
                ...
                "render": function (data, type, full, meta) {
                    //查看订单详情
                    var url = Context.PATH + "/order/query/detail.html?orderid=" + full.id + "&op=fillup";
                ...
                "render": function (data, type, full, meta) {
                    ...
                    if (full.status == '4') {//已关联合同
                        ...
                        if (full.payStatus == 'REQUESTED') {
                            return renderStatus("7");//已申请付款
                        }
                        if (full.payStatus == 'APPROVED') {
                            return renderStatus("8");//已通过付款申请
                        }
                        if(full.payStatus == 'APPLYPRINTED'){
                            return renderStatus("11");//已打印付款申请
                        }
