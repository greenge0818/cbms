...
/**
 * 创建超链接
 * @param c 样式名,id 订单ID,status 订单变更状态
 * @param text 显示文本
 */
function createChangeAele(c,id,status, text) {
    return '<a class="'+c+'" orderId="' + id + '" type = "' + status + '" href="javascript:;">' + text + '</a>';
}
/**
 * 返回打印变更合同URL
 * @param changeOrderid
 */
function getPrintChangeContractURL(changeOrderid) {
    return Context.PATH + "/order/changecontract/print/contract.html?orderChangeId=" + changeOrderid;
}