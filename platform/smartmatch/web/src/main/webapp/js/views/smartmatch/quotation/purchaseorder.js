$(document).ready(function () {
    $.post(Context.PATH + '/common/organizationList.html','',function(result){
        ...
        if(result.success){
            for (var i = 0; i < result.data.length; i++) {
                $("#sorganizationHidden").append("<option value='" + result.data[i].id + "'>" + result.data[i].name + "</option>");
            }
        }
    });
    $(document).on("click", "#btnassign", function () {
        var url = Context.PATH + '/smartmatch/purchaseorder/push.html';
        var quotationOrderId = $("#quotation_orderid").val();
        var org_id = 0;
...
function loadUserData(orgId,roleType,callback,inputBox){
    $.ajax({
        ...
        data: {orgId: orgId,roleType:roleType,type:"pushService"},
        dataType: "json",
        error: function (s) {
        }
