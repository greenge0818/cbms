...
function loadMaterial(inputBox) {
	...
	if(arguments[1] > 0){
		var showPlace = $(inputBox).closest('.addOrUpdateResClass').find('#' + materialLayerId);
...
function loadAttribute(inputBox) {
    ...
    remarks = '<lable id="remarkLab" class="search-radio-bar craft-bar">备注：<input id="detailRemark" name="detailRemark" style="width:150px" class="" type="text"></lable>';
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $(showPlace).find("div.attrs").append(html+remarks);
...
function loadFactory(input) {
    $.ajax({
        ...
        success: function (data) {
            ...
            var factory_nav_link=$(input).closest('#m-search').find('.factory_layer .factory-nav-link');
            if (data.statusCode === 0) {
                $(showPlace).empty();
...
function getFactoryRenderHtml(factoryList,rowSize) {
    var temp_html='';
    for(var i=0;i<factoryList.length;i++){
        if (i % rowSize === 0) {
            temp_html += '<div class="textures-con-bar-list bder-b-dashed">';
        }
        temp_html += '<span><a href="javascript:;" factoryId="' + factoryList[i].uuid + '" class="">' + factoryList[i].name + '</a></span>';
        if ((i + 1) % rowSize === 0 || (i + 1) == factoryList.length) {
            temp_html += "</div>";
        }
    }
    return temp_html;
...
function enableSpec1Event(){
    ...
    $(document).on("click","#showLayer_" + spec + " .textures-list-first",function () {
        ...
        if ($(this).attr("checked")) {
        	...
        	if($(this).closest(".show-layer").find("a.hover").length >= 10){
        		cbms.alert("最多选择10种规格");
        		return ;
        	}
        ...
        //if (prcsteel.checkInput(from, to)) {
            if ($.trim($(from).val()) === "" && $.trim($(to).val()) === "") {
                ...
                if ($.trim($(from).val()) !== "" && $.trim($(to).val()) !== "") {//起始结束都不为空
                    minVal = $(from).val();
                    maxVal = $(to).val();
                    if(minVal!=null && minVal!="null"&&maxVal!=null && maxVal!="null"){
...
function enableFactoryEvent(){
    ...
    $(document).on("click", ".factory_layer .textures-con span a", function () {
        if ($(this).hasClass("hover")) {
        	...
        	//显示钢厂选中数量为10家
			if($(this).closest("#showLayer_factory").find("a.hover").length >= 10){
				cbms.alert("最多选择10家钢厂！");
				return ;
			}
...
function enableMaterialEvent() {
    ...
    $(document).on("click", "#" + materialLayerId + " span a",function () {
    	...
    	//<!--if 点击修改or添加时弹窗引用 S-->
    	if($(this).closest("#layerSearchId").length > 0){
    		
        	var materId = $(this).attr("materialid"), materNm = $(this).attr("materialname");
        	
        	$(this).closest(".show-layer").prev("#addorupdate-material").val(materNm);
        	$(this).closest(".show-layer").prev("#addorupdate-material").attr("val",materId);
        	$(".show-layer").hide();
        	cbms.stopF(e);
            return false;
    	}
